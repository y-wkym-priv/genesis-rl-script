...
COPY Genesis/docker/build_luisa.sh /workspace/build_luisa.sh
...
COPY Genesis/docker/10_nvidia.json /usr/share/glvnd/egl_vendor.d/10_nvidia.json
COPY Genesis/docker/nvidia_icd.json /usr/share/vulkan/icd.d/nvidia_icd.json
COPY Genesis/docker/nvidia_layers.json /etc/vulkan/implicit_layer.d/nvidia_layers.json

RUN pip3 install rsl-rl-lib==2.2.4
